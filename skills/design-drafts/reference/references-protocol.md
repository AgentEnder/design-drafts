...
The structured output of the `design-drafts:brief` skill. Treat this as the source of truth for the draft's intent: audience, tone, hard constraints, must-not-look-likes.
...
The structured output of the `design-drafts:explore` skill. It contains the user's premise, axes-with-concept-options (some marked `[picked]`), captured quotes, and a list of references gathered during the brainstorm.
...
- **`design-drafts:explore`** — writes `explore.md`. Captures `links.md` entries and `inspiration/` images on the fly via `design-drafts ref add` as the user mentions URLs and screenshots during the brainstorm.
- **`design-drafts:brief`** — writes `brief.md`. Reads `explore.md` if present and seeds the interview from the picks; otherwise cold-interviews. Continues to capture references inline via `design-drafts ref add`.
